module "cluster" {
  source                 = "../modules/base"
  environment            = "staging"
  cluster_name           = "{{ project_dash }}-staging"
  domain_name            = "staging.{{ domain_name }}"
  api_domain_name        = "api.staging.{{ domain_name }}"
  cluster_domain_name    = "k8s.staging.{{ domain_name }}"
  argocd_domain_name     = "argocd.staging.{{ domain_name }}"
  prometheus_domain_name = "prometheus.staging.{{ domain_name }}"
  control_plane = {
