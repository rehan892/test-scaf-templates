...
variable "api_domain_name" {
  type    = string
  default = "api.{{ domain_name }}"
}

variable "cluster_domain_name" {
  type    = string
  default = "k8s.{{ domain_name }}"
}

variable "nextjs_domain_name" {
  type    = string
  default = "nextjs.{{ domain_name }}"
}

variable "argocd_domain_name" {
  type    = string
  default = "argocd.{{ domain_name }}"
}


variable "prometheus_domain_name" {
  type    = string
  default = "prometheus.{{ domain_name }}"
}
...
variable "frontend_ecr_repo" {
  ...
  type        = string
  default     = "{{ project_dash }}-sandbox-frontend"
}
{% endif %}
...
variable "backend_ecr_repo" {
  ...
  type        = string
  default     = "{{ project_dash }}-sandbox-backend"
}
