module "cluster" {
  ...
  api_domain_name        = "api.sandbox.{{ domain_name }}"
  cluster_domain_name    = "k8s.sandbox.{{ domain_name }}"
  argocd_domain_name     = "argocd.sandbox.{{ domain_name }}"
  prometheus_domain_name = "prometheus.sandbox.{{ domain_name }}"
  control_plane = {
