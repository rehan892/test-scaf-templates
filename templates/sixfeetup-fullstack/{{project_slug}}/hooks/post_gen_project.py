...
create_nextjs_frontend = "{{ create_nextjs_frontend }}"
...
def remove_celery_files():
    file_names = [
        os.path.join("backend", "{{ project_slug }}", "celery.py"),
        os.path.join("backend", "{{ project_slug }}", "users", "tasks.py"),
        os.path.join(
            "backend",
            "{{ project_slug }}",
            "users",
...
def remove_challenge_files():
    challenge_dir = os.path.join("backend", "{{ project_slug }}", "challenge")
...
def main():
    ...
    if "{{ source_control_provider }}" != "bitbucket.org":
        remove_bitbucket_files()

    if "{{ source_control_provider }}" != "github.com":
        remove_github_files()

    if "{{ use_sentry }}" != "True":
        remove_sentry_files()

    if "{{ create_nextjs_frontend }}" != "True":
